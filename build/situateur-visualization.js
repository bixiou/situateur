...
var Curve = require('svg/curve'),
  ...
  revdispDistribution = require('revdisp-distribution.json'),
  salDistribution = require('sal-distribution.json'),
  YAxis = require('svg/y-axis');


var mountNode = document.getElementById('simulator-container');
React.renderComponent(Simulator(null), mountNode);

var Simulator = React.createClass({displayName: 'Simulator',
    render: function() {
        var value = 1000;
        var curveLabel, hintFormat, pointLabel, points, rightPanelWidth;
...
var SituateurVisualization = React.createClass({displayName: 'SituateurVisualization',
  propTypes: {
  ...
  render: function() {
    ...
    return (
      React.DOM.div(null, 
        React.DOM.svg({height: this.props.height, width: this.props.width}, 
          React.DOM.g({transform: 
          ...
          React.DOM.g({transform: 'translate(' + this.props.yAxisWidth + ', ' + this.props.legendHeight + ')'}, 
          ...
          React.DOM.g({transform: 'translate(' + this.props.yAxisWidth + ', 0)'}, 
            Legend({color: "rgb(31, 119, 180)"}, this.props.curveLabel), 
            React.DOM.g({transform: 'translate(' + 12 * 0.7 * this.props.curveLabel.length + ', 0)'}, 
              Legend({color: "rgb(166, 50, 50)"}, this.props.pointLabel)
            )
          )
        ), 
        
          React.DOM.p({className: "well", style: {textAlign: 'center'}}, 
