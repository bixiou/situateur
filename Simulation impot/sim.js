...
//            traceGrapheComplet('repartitionComplet');
    ...
    function ajusteCourbe(enregistrer) {
        ...
        function verifieConditions(enregistrer){
            var iA,iF,alerte=1;
//            if (T<0) {
//                alert('Ta proposition inclue un transfert d\'argent des pauvres vers les riches, nous ne la retenons pas.');
//                alerte=0;
//            }
//            for (i=1;i<n;i++) {
//                if (tauxAvecA[i]>tauxAvecA[i+1]) {
//                    alert('Ta proposition ne conduit pas à un impôt progressif : en effet, les revenus au seuil n°'+i+' sont imposés à '+tauxAvecA[i].toFixed(1)+'% tandis qu\'au seuil suivant ils sont imposés à '+tauxAvecA[i+1].toFixed(1)+'%. Nous ne retenons donc pas ta proposition.');
//                    alerte=0;
//                    break;
//                }
//            }
...
//                    alert('Ta proposition n\'est pas socialement juste : compare l\'impôt actuel ('+iA.toFixed(1)+'%) et l\'impôt proposé ('+iF.toFixed(1)+'%) pour un revenu de '+donnees['revenuFinal'][i].revenu+'€ par exemple. Nous ne retenons pas ta proposition.');
//                    alerte=0;
//                    break;
//                }
//            }
            if (alerte) {
                if (enregistrer) {
                    enregistreSimulation();
                    $('#enregistrer').hide();
                    alert('Félicitations ! Ton barème a été enregistré !');
                } else {
                    alert('Bien joué : ta proposition vérifie les contraintes !');
                    $('#enregistrer').show();
                }
            }
            return alerte;
        ...
        function enregistreSimulation() {
         $.post( 'index.php',
                { 'bareme': seuilsAvecA.concat(nouvellesValeursSeuils).toString() },
                 function(info){});
        }
    ...
    function simuleImpot() {
        ...
        for (j=0;j<nbSliders;j++) {
            ...
            sliders['slider'+j].attachEvent("onSlideEnd",function(pos,slider){
                traceRepartitionImpot(seuils,valeursSeuils);
                $('#revenuActuel').trigger('change');
                if (donnees['tauxImpot']!=undefined) {
                    taux=tauxImposition();
                    chargeTauxImpot(taux);
                    chargeTableImpot(1);
                    graphe['tauxImpot'].getDataSourceSettings().reloadData();
                    graphe['tauxImpot'].getSeries().getItem(0).setText("taux d'imposition après la réforme");
                }
            });
        }
        ...
        function enregistrePosition(j) {
                ...
                valeursSeuils[j]=pos;
                if (j+1 == nbSliders) {
                    pourcent = (100 * (1 - pos / donnees['revenuImposable'][donnees['revenuImposable'].length-1].revenu)).toFixed(1);
                    $('#nouveauTauxMax').text(pourcent.replace('.',','));
                }   
            });
        }
        ...
        function chargeTableImpot(init) {
            ...
            if (!init) {
                ...
                for (i=1;i<nbSliders;i++){
                    ...
                    chargeTaux(i, tauxI, tauxP);
                }
            }
        }

        function chargeTaux(i, tauxI, tauxP) {
            if (tauxI == -5) {
                $('#ri'+i).text((seuils[i]/100).toFixed(0)*100);
                $('#ta'+i).text(tauxP.toFixed(1).replace('.',',')+'%');
            } else {                
                $('#ri'+i).text((seuils[i]/100).toFixed(0)*100);
                $('#ta'+i).text(tauxI.toFixed(1).replace('.',',')+'%');
                $('#nt'+i).text(tauxP.toFixed(1).replace('.',',')+'%');
            }
            left = 98 + 48 * i;
            $('#ri'+i).css('left',left + 'px');
            $('#ta'+i).css('left',left + 'px');
            $('#nt'+i).css('left',left + 'px');
            $('#nf'+i).css('left',left + 'px');
        }
    }
    
    function compareRevenu() {
        $('#revenuActuel').change(function() {
            var revenu=$('#revenuActuel').val();
            revenu=revenu2(revenu,'revenuFinal','nouvelImpot').toFixed(0);
            $('#nouveauRevenu').text(revenu);
        });
    }
    ...
    //configuration de la mise en forme
        ...
        if (nom=='nouvelImpot') {
            graphe[nom].getAxisY().setTextColor('#00FFFFFF');
            graphe[nom].getAxisX().setTextColor('#00FFFFFF');
    ...
    //configuration de la mise en forme   
        ...
        graphe[nom].getLegendBox().setFont("11pt Arial");
        if (nom=='tauxImpot') {
            graphe[nom].getAxisX().getGrids().getMajor().setVisible(true);
            graphe[nom].getAxisY().getGrids().getMajor().setVisible(true);
            titreX = new cfx.TitleDockable();
            ...
            graphe[nom].getLegendBox().setTextColor("#008000");
            graphe[nom].getLegendBox().setMarginX(280);
            graphe[nom].setBackColor('#FFFFFF');
        }
...
//$(document).ready(function decalerSliders(px) {
//        $('.slider').each( function () { 
//            hauteur = $(this).css('top');
//            $(this).css('top', (parseInt(hauteur.replace('px','')) + px) + 'px');
//        });
//}(0) );  
